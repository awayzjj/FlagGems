...
# CUDA-style radix select (dim mid-range + large flat). Needs atomic_min i32.
...
def flat_radix_find_index_kernel(
    ...
    if tl.load(valid_count) > 0:
        ...
        desired = tl.load(state + 0).to(utype)
        keys = _to_order_key(vals, valid)
        # i32 is enough: flat radix path guarantees n <= INT32_MAX.
        # Iluvatar does not support atomic_min on i64.
